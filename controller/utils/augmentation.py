...
kg_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'central_kg.json')

# Fresh graph if not existing
if not Path(kg_file).exists():
    G = kg_handler.build_initial_graph()

def augmentUserMessage(user_message, type="chat", search_res=""):
    """
